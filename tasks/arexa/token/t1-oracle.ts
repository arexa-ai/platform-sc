...
arexaScope
	.task("token:oracle:approve", "Approve a spender to spend some amount of token from your address")
...
arexaScope.task("token:oracle:getCurrentTokenId", "Get the most current ORACLE subscript token Id").setAction(async (_taskArg, hre) => {
	const arexa = await getAREXASmartContracts(hre);
...
arexaScope
	.task("token:oracle:balance", "Gets the balance of an address of a token")
...
arexaScope
	.task("token:oracle:buy", "Buy ORACLE Subscripton token")
...
arexaScope
	.task("token:oracle:transfer", "Transfer ORACLE subscription token")
...
arexaScope
	.task("token:oracle:transferFrom", "")
