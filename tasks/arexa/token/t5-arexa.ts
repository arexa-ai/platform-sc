...
arexaTokenScope
	.task("5-magic:approve", "Approve a spender to spend some amount of token from your address")
	...
	.setAction(async (params: Pick<All, "signer" | "address" | "currvalue" | "newvalue">, hre) => {
		...
		const result = await call(hre, contract.approve(params.address, arexa.const.MAGIC_TOKEN_ID, params.currvalue, params.newvalue));
		await hre.run("print", { message: ` TX: ${result.hash}` });
	});

arexaTokenScope
	.task("5-magic:allowance", "Check allowance of a spender to spend some amount of USDT from an address")
	...
	.setAction(async (params: Pick<All, "spender" | "holder">, hre) => {
		const arexa = await getAREXASmartContracts(hre);
		const result = await arexa.pfmTokenAllowanceFacet.allowance(params.holder, params.spender, arexa.const.MAGIC_TOKEN_ID);
		await hre.run("print", {
...
arexaTokenScope
	.task("5-magic:balance", "Gets the balance of an address of a token")
	.addParam("address", "Which address of balance?", undefined, types.string)
	.setAction(async (params: Pick<All, "address">, hre) => {
		const arexa = await getAREXASmartContracts(hre);
		const result = await arexa.pfmTokenFacet.balanceOf(params.address, arexa.const.MAGIC_TOKEN_ID);
		await hre.run("print", {
...
arexaTokenScope
	.task("5-magic:buy", "Buy AREXA AI token")
	.addParam("signer", "Index of Signer", undefined, types.int)
	.setAction(async (params: Pick<All, "signer" | "value">, hre) => {
		const arexa = await getAREXASmartContracts(hre);
		const contract = arexa.platformFacet.connect(arexa.signers[params.signer]);
		const result = await call(hre, contract.buyMagic100Token());
		await hre.run("print", { message: ` TX: ${result.hash}` });
	});

arexaTokenScope
	.task("5-magic:transfer", "Transfer AREXA AI token")
	.addParam("signer", "Index of Signer", undefined, types.int)
	...
	.setAction(async (params: Pick<All, "signer" | "address" | "value">, hre) => {
		...
		const result = await call(
			hre,
			contract.safeTransferFrom(signer.address, params.address, arexa.const.MAGIC_TOKEN_ID, params.value, []),
		);
		await hre.run("print", { message: ` TX: ${result.hash}` });
	});

arexaTokenScope
	.task("5-magic:transferFrom", "On behalf trasnfer AREXA AI subscription token")
	.addParam("signer", "Index of Signer", undefined, types.int)
	...
	.setAction(async (params: Pick<All, "signer" | "from" | "to" | "value">, hre) => {
		...
		const signer = arexa.signers[params.signer];
		const contract = arexa.pfmTokenFacet.connect(signer);
		const result = await call(hre, contract.safeTransferFrom(params.from, params.to, arexa.const.MAGIC_TOKEN_ID, params.value, []));
		await hre.run("print", { message: ` TX: ${result.hash}` });
	});
