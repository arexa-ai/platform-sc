...
describe("ArexaPlatform - T1 - Oracle Token", function () {
	describe("User buy T1 (Oracle Token)", async function () {
		//
		it("Admin can create new subscription", async function () {
			...
			let tokenId = await arexa.platformFacet.getCurrentSubscriptionTokenId(arexa.const.SUBSRIPTION1_TOKEN_TYPE);
			expect(tokenId).to.be.equal(0);

			let tokenResult = await platformAdminFacetOwn.createSubscription(arexa.const.SUBSRIPTION1_TOKEN_TYPE, 2023, 12, 1000, 15, 1000);
			await expect(tokenResult).not.to.be.reverted;

			tokenId = await arexa.platformFacet.getCurrentSubscriptionTokenId(arexa.const.SUBSRIPTION1_TOKEN_TYPE);
			expect(tokenId).to.be.equal(120231201);

			tokenResult = await platformAdminFacetOwn.createSubscription(arexa.const.SUBSRIPTION1_TOKEN_TYPE, 2023, 12, 1000, 15, 1000);
			await expect(tokenResult).not.to.be.reverted;

			tokenId = await arexa.platformFacet.getCurrentSubscriptionTokenId(arexa.const.SUBSRIPTION1_TOKEN_TYPE);
			expect(tokenId).to.be.equal(120231202);
		});
		...
		it("Admin could not create new subscription for old month", async function () {
			...
			const resErr = platformAdminFacetOwn.createSubscription(arexa.const.SUBSRIPTION1_TOKEN_TYPE, 2023, 11, 1000, 15, 1000);
